...
async debugTrialClasses(): Promise<any[]> {
  try {
    console.log('🔍 [DEBUG] Checking TrialClass collection state...');
    
    
    const totalCount = await TrialClass.countDocuments();
    console.log('🔍 [DEBUG] Total trial classes in collection:', totalCount);
    
   
    const allDocs = await TrialClass.find({}).limit(5);
    console.log('🔍 [DEBUG] First 5 trial classes:');
    allDocs.forEach((doc, index) => {
      console.log(`  ${index + 1}. _id: ${doc._id}`);
      console.log(`     student: ${doc.student} (type: ${typeof doc.student})`);
    ...
    console.error('❌ [DEBUG] Error checking trial classes:', error);
    throw error;
  }
}
