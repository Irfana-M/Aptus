...
//   if (pathRole === ROLES.STUDENT && studentLoading)  {
//     return (
...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({
  ...
  const getToken = (): string | null => {
    ...
    const token =
      localStorage.getItem(`${roleKey}_accessToken`) ||
  ...
  }, [pathRole, studentProfile, studentLoading, dispatch, path]);
  ...
  const isCallRoute =
    (path.includes("/trial-class/") || path.includes("/video-call")) &&
    path.includes("/call");

  if (isCallRoute) return children;
  ...
  if (!authContext.validateRoleForPath(path)) {
    ...
    if (role === ROLES.MENTOR) return <Navigate to={ROUTES.MENTOR.DASHBOARD} replace />;
  }
  ...
  if (pathRole === ROLES.ADMIN && adminState.loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <Loader size="lg" text="Loading admin console..." />
      </div>
    );
  }
