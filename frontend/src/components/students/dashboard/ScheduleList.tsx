...
import {
  ...
  reportAbsence,
} from "../../../features/session/sessionThunk";
...
const ScheduleList: React.FC = () => {
  ...
  // Selectors
  const { trialClasses } = useSelector(
    (state: RootState) => state.studentTrial,
  );
      ...
      if (trial.preferredDate) {
        ...
        if (isSameDay(trialDate, date)) {
          scheduleItems.push({
            ...
            type: "Trial Class",
            subject:
              typeof trial.subject === "object" && trial.subject
                ? trial.subject.subjectName
                : String(trial.subject),
        ...
        <div className="grid grid-cols-7 gap-2 mb-4">
          {weekDays.map((day, index) => {
            ...
            return (
              ...
              >
                <p
                  className={`text-xs mb-1 group-hover:text-teal-600 ${isSelected ? "text-teal-600 font-semibold" : "text-gray-500"}`}
                >
                  {dayName}
                </p>
                <div
                  className={`w-8 h-8 mx-auto rounded-lg flex items-center justify-center text-sm transition-colors ${
                    isSelected
                      ...
                      : "text-gray-700 bg-gray-50 group-hover:bg-teal-50"
                  }`}
                >
                  {dayNumber}
                </div>
          ...
          {loading ? (
            ...
            schedule.map((item, idx) => {
              ...
              const canReschedule =
                !item.isTrial &&
                (isRescheduling ||
                  (item.session?.cancelledBy === "mentor" &&
                    item.status === "cancelled"));

              return (
                <div
                  key={`${item.id}-${idx}`}
                  className={`flex gap-3 items-start group p-3 rounded-lg border transition-all ${
                    isCancelled
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-lg flex items-center justify-center font-bold text-lg flex-shrink-0 transition-colors ${
                      item.isTrial
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      <div>
                        <p
                          className={`font-semibold truncate ${isCancelled ? "text-red-900" : "text-gray-900"}`}
                        >
                          {item.subject}
                        </p>
                        <p className="text-xs text-gray-500 truncate">
                          {item.type} •{" "}
                          <span
                            className={`capitalize ${isCancelled ? "text-red-500 font-medium" : ""}`}
                          >
                            {item.status.replace("_", " ")}
                          </span>
                          {item.session?.cancellationReason &&
                            ` (${item.session.cancellationReason})`}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {item.link && !isCancelled && !isRescheduling && (
                          <button
                            onClick={() => {
                              if (item.isTrial) {
                                navigate(`/trial-class/${item.id}/call`);
                                return;
                              }

                              // === REGULAR CLASS ONLY - Safe TS handling ===
                              if (!item.session || !item.session.startTime) {
                              ...
                              if (canJoin) {
                                // Safely grab the aliased id rather than MongoDB _id
