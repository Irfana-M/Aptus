      ...
      <div className="space-y-3">
        {upcomingClass ? (
          <div
            ...
            onClick={() => {
              if (upcomingClass.title.startsWith("Trial")) {
                navigate(`/trial-class/${upcomingClass.id.split("-")[0]}/call`);
                return;
              }

              // Regular class (one-to-one or group) - Safe TS handling
              const session = sessions.find((s) => s.id === upcomingClass.id);
              ...
              const canJoin =
                now >= new Date(start.getTime() - 30 * 60 * 1000) && now <= end;

              if (canJoin) {
                // Exclusively use standard .id mapped alias
                window.open(`/session/${session.id}/call`, "_blank");
              } else {
                navigate("/student/classroom");
              }
            }}
