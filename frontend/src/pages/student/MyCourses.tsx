        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Once you subscribe, our admin reviews your schedule and matches
              you with a mentor.
            </p>
          </div>
          <Button
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => (
              <div
                key={course._id}
                className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group"
              >
                <div
                  className={`absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 opacity-5 group-hover:opacity-10 transition-opacity`}
                >
                  <BookOpen size={128} />
                </div>
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <span
                    className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border flex items-center gap-1.5 ${course.isActive ? "bg-green-100 text-green-700 border-green-200" : "bg-slate-100 text-slate-700 border-slate-200"}`}
                  >
                    {course.isActive ? (
                      <CheckCircle size={16} />
                    ) : (
                ...
                <p className="text-slate-400 text-sm mb-4 flex items-center gap-1.5">
                  <Clock size={14} />
                  {course.schedule?.days?.join(", ") || "TBA"} •{" "}
                  {course.schedule?.timeSlot || "TBA"}
                </p>

                <div className="bg-slate-50 rounded-2xl p-4 space-y-4">
                  <div className="flex items-center justify-between text-[10px] font-black text-slate-400 uppercase tracking-wider">
                    <span>
                      Grade {course.grade?.grade || "N/A"} • {course.courseType}
                    </span>
