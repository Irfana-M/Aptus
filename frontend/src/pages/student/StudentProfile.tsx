...
const StudentProfile: React.FC = () => {
    ...
    if (profile) {
        ...
        // Fallback to basic user info if profile not fetched or incomplete
         setProfileData(prev => ({
            ...
            profileImageUrl: user.profileImageUrl || (user as unknown as { profilePicture?: string }).profilePicture || prev.profileImageUrl,
        }));
    }
  }, [profile, user]);
  ...
  const handleImageUpload = (type: 'profile') => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <StudentLayout title="My Profile">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                {isEditing && (profile?.isProfileCompleted || user?.isProfileComplete) && (
                     <button 
              ...
              {isEditing ? (
                  ...
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                      ...
                      <div className="mb-8">
                        ...
                        <FormField label="Full Name" type="text" value={profileData.fullName} onChange={handleChange('fullName')} required />
                        <FormField label="Email ID" type="email" value={profileData.emailId} onChange={handleChange('emailId')} required />
                        <FormField label="Phone Number" type="tel" value={profileData.phoneNumber} onChange={handleChange('phoneNumber')} />
                        <div className="grid md:grid-cols-2 gap-4">
                          <FormField label="Date of Birth" type="date" value={profileData.dateOfBirth} onChange={handleChange('dateOfBirth')} />
                          <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">Gender</label>
                            <select value={profileData.gender} onChange={(e) => handleChange('gender')(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white">
                        ...
                        <FormField label="Age" type="number" value={profileData.age} onChange={handleChange('age')} />
                      </div>

                      {/* Address */}
                      <div className="mb-8">
                        <h2 className="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b">Address</h2>
                        <FormField label="Address" type="textarea" value={profileData.address} onChange={handleChange('address')} rows={3} />
                        <div className="grid md:grid-cols-2 gap-4">
                          <FormField label="Country" type="text" value={profileData.country} onChange={handleChange('country')} />
                          <FormField label="Postal Code" type="text" value={profileData.postalCode} onChange={handleChange('postalCode')} />
                        </div>
                      </div>

                      {/* Parent/Guardian Details */}
                      <div className="mb-8">
                        <h2 className="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b">Parent/Guardian Details</h2>
                        <FormField label="Name" type="text" value={profileData.parentName} onChange={handleChange('parentName')} />
                        <FormField label="Email" type="email" value={profileData.parentEmail} onChange={handleChange('parentEmail')} />
                        <FormField label="Phone" type="tel" value={profileData.parentPhone} onChange={handleChange('parentPhone')} />
                        <FormField label="Relationship" type="text" value={profileData.relationship} onChange={handleChange('relationship')} />
                      </div>

                      {/* Academic Preferences */}
                      <div className="mb-8">
                        <h2 className="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b">Academic Preferences</h2>
                        <FormField label="Institution" type="text" value={profileData.institution} onChange={handleChange('institution')} />
                        
                        <div className="grid md:grid-cols-2 gap-4">
                            <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">Grade <span className="text-red-500 ml-1">*</span></label>
                            <select value={profileData.grade} onChange={(e) => handleChange('grade')(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white" disabled={gradesLoading}>
                            ...
                            <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">Syllabus <span className="text-red-500 ml-1">*</span></label>
                            <select value={profileData.syllabus} onChange={(e) => handleChange('syllabus')(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white" disabled={!profileData.grade}>
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                       <div className="bg-gray-50 rounded-xl p-6 border border-gray-100">
                           <h3 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">Personal Info</h3>
                       ...
                       <div className="bg-gray-50 rounded-xl p-6 border border-gray-100">
                           <h3 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">Address</h3>
                       ...
                       <div className="bg-gray-50 rounded-xl p-6 border border-gray-100">
                           <h3 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">Guardian Info</h3>
                           ...
                           <InfoRow label="Relationship" value={profileData.relationship} />
                       </div>

                       <div className="bg-gray-50 rounded-xl p-6 border border-gray-100">
                           <h3 className="text-sm uppercase tracking-wider text-gray-500 font-bold mb-4">Academic Info</h3>
          ...
          {/* Right Column - Uploads (Only visible in edit mode or always?) -> Let's keep it visible but maybe disabled state? */}
          {/* For now, keeping it visible as user might want to check profile pic */}
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-8">
              <div className="text-center mb-6">
                <div className="relative inline-block group">
                  {profilePicture ? (
                    <img src={URL.createObjectURL(profilePicture)} alt="Profile" className="w-32 h-32 rounded-full object-cover mx-auto ring-4 ring-offset-2 ring-indigo-50" />
                  ) : profileData.profileImageUrl ? (
                    <img src={profileData.profileImageUrl} alt="Profile" className="w-32 h-32 rounded-full object-cover mx-auto ring-4 ring-offset-2 ring-indigo-50" />
                  ) : (
                  ...
                  {isEditing && (
                      <label className="absolute bottom-0 right-0 bg-white p-2 rounded-full shadow-lg border border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors">
                        ...
                        <input type="file" accept="image/*" onChange={handleImageUpload('profile')} className="hidden" />
                      </label>
                  )}
                </div>
                
                <h3 className="text-lg font-bold text-gray-800 mt-4">{profileData.fullName || 'Student Name'}</h3>
                <p className="text-sm text-gray-500">{profileData.emailId || 'student@example.com'}</p>
                {profileData.grade && (
