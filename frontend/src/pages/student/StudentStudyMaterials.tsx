...
const StudentStudyMaterials: React.FC = () => {
  ...
  const [submissionModal, setSubmissionModal] = useState<{
    show: boolean;
    assignment: StudyMaterial | null;
  }>({ show: false, assignment: null });
  const [submissionDetails, setSubmissionDetails] = useState<{
    [key: string]: AssignmentSubmission | null;
  }>({});
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setMaterials(
        allMaterials.filter(
          (m: StudyMaterial) => m.materialType === "study_material",
        ),
      );
      ...
      const submissionPromises = assignmentData.map(
        async (a: StudyMaterial) => {
          try {
      ...
      results.forEach(
        (r: { id: string; submission: AssignmentSubmission | null }) => {
          details[r.id] = r.submission;
        },
      );
                ...
                <div className="space-y-4">
                  {materials
                    ...
                    .map((m) => (
                      <div
                        key={m._id}
                        className="p-5 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between hover:border-indigo-200 transition-colors"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              {m.subjectId?.subjectName || "General"} • by{" "}
                              {m.mentorId?.fullName} •{" "}
                        ...
                        <button
                          onClick={() =>
                            handleDownload(m.fileUrl, m.originalName)
                          }
          ...
          assignments.length > 0 ? (
            <>
              <div className="space-y-4">
                {assignments
                  .slice(
                    (assignmentsPage - 1) * ITEMS_PER_PAGE,
                    assignmentsPage * ITEMS_PER_PAGE,
                  )
                  .map((a) => {
                    console.log("Single Assignment:", a);
                    const dueDate = a.assignmentDetails?.dueDate || (a as any).dueDate;
                    const dueStatus = a.assignmentDetails?.dueDate
                      ? getDueStatus(a.assignmentDetails.dueDate)
                      : null;
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex items-start gap-4 flex-1">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-3">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded-full text-xs font-bold ${subStatus.color}`}
                                >
                                  {subStatus.text}
                                </span>
                              </div>
                              ...
                              <div className="flex items-center gap-4 mt-3 text-xs text-slate-500">
                                ...
                                {dueStatus && (
                                  <span
                                    className={`px-2 py-0.5 rounded-full font-bold ${dueStatus.color}`}
                                  >
                                    {dueStatus.text}
                                  </span>
                                )}
                          ...
                          <div className="flex flex-col gap-2 items-end ml-4">
                            <button
                              onClick={() =>
                                handleDownload(a.fileUrl, a.originalName)
                              }
