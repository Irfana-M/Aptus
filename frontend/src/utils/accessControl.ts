...
export const resolveAccessState = (user: any): AccessState => {
  ...
  if (!user.isProfileCompleted && !user.isProfileComplete) return AccessState.PROFILE_COMPLETION;
  if (!user.isTrialCompleted) return AccessState.TRIAL_BOOKING;
  
  const isSubscribed = user.hasPaid || user.isPaid || (user.subscription?.status === 'active');
